...
public class CompetitionDijkstra {
	...
	public int timeRequiredforCompetition() {
		...
		for (int i = 0; i < city.length; i++) {
			boolean[] interChecked = new boolean[city.length];
			interChecked[i] = true;
			while (true) {
				int x = -1;
				for (int j = 0; j < city.length; j++)
					if ((interChecked[j] == false) && (city[i][j] != Integer.MAX_VALUE)) {
						x = j;
						break;
					}
				if (x == -1)
				...
				interChecked[x] = true;
				for (int j = 0; j < city.length; j++)
					if (city[i][x] + city[x][j] < city[i][j]) {
						city[i][j] = city[i][x] + city[x][j];
						interChecked[j] = false;
					}
			}
		}
